...
class ERTMSOnBoardSystem:
    ...
    def _procedure_d32_a33_a34_position_report(self):
        if self.position_status == DataStatus.VALID:
            # If position is already valid, we skip the RBC pos report logic
            # but logically in BPMN we might still store acceptance.
    ...
    def _procedure_s21_s25_l2_grant(self):
        ...
        if response == 'ma_sr':
            ...
            self._log_activity("som_awaitack_dmi_1") # Assuming FS might require ack in some flows, or implicit. BPMN shows awaitack after all grants.
            self._log_activity("som_chmod_evc_1")
            self.current_mode = Mode.FULL_SUPERVISION
            return None
        return 'S10'
