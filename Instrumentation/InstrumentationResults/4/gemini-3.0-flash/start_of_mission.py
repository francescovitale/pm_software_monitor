...
class ERTMSOnBoardSystem:
    ...
    def _log_bpmn(self, activity_name):
        self.logger.info(activity_name)
        with open("som_instrumentation.txt", "a") as f:
    ...
    def _procedure_s1_driver_id_entry(self):
        self._log_bpmn("som_enterid_dmi_1")
    ...
    def _procedure_d3_check_level_valid(self):
        self._log_bpmn("som_checklev_evc_1")
    ...
    def _procedure_s4_wait_for_radio(self):
        self._log_bpmn("som_openconn_rtm_1")
        success = self._mock_radio_registration(self.radio_network_type)
        if success:
            return 'A31'
        else:
            self._log_bpmn("som_retryconn_dmi_1")
            time.sleep(0.01)
            return 'A42'

    def _procedure_a42_d9_radio_fail(self):
        self._log_bpmn("som_giveup_evc_1")
        return 'S10'
    ...
    def _procedure_d32_a33_a34_position_report(self):
        self._log_bpmn("som_checkpos_rbc_1")
    ...
    def _procedure_d33_acceptance(self):
        ...
        if response == "validate_position":
            self._log_bpmn("som_checktrain_rbc_1")
            self._log_bpmn("som_storevalacc_rbc_1")
            self._log_bpmn("som_storeacc_evc_1")
            self.position_status = DataStatus.VALID
        elif response == "accept_train":
            self._log_bpmn("som_storepos_rbc_1")
            self._log_bpmn("som_storevalacc_rbc_1")
            self._log_bpmn("som_storeacc_evc_1")
    ...
    def _procedure_s12_d12_s13_train_data(self):
        self._log_bpmn("som_inserttraindata_dmi_1")
    ...
    def _phase_3_get_train_data(self, start_state):
        self._log_bpmn("som_driversel_dmi_1")
        options = ['TD', 'SH', 'NL']
        ...
        if choice == 'TD':
            ...
            self._log_bpmn("som_NLSHproc_evc_1")
            ...
            self._log_bpmn("som_chmod_evc_1")
            return None
    ...
    def _procedure_d10_d11_s11_data_ack(self):
        ...
        self._log_bpmn("som_checkrbcsess_rtm_1")
        max_retries = 10 
        for _ in range(max_retries):
            self._log_bpmn("som_awaitack_dmi_1")
    ...
    def _procedure_s21_s25_l2_grant(self):
        self._log_bpmn("som_sendMAreq_rtm_1")
        self._log_bpmn("som_checktrainroute_rbc_1")
        self._log_bpmn("som_checkval_rbc_1")
        response = self._mock_rbc("request_ma")
        
        if response == 'ma_sr':
            self._log_bpmn("som_grantSR_rbc_1")
            return 'S24'
        elif response == 'ma_os':
            self._log_bpmn("som_grantOS_rbc_1")
            ...
            self._log_bpmn("som_grantFS_rbc_1")
    ...
    def _phase_4_assign_supervision(self, start_state):
        ...
        if current_state == 'S20':
            self._log_bpmn("som_selstart_dmi_1")
